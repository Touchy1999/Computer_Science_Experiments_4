...
# mini_batch
def mini_batch():
  global W_2, W_1, b_1, b_2, alpha, M, C, d, learning_rate
  dW_1 = np.array(np.zeros((M, d)))
  dW_2 = np.array(np.zeros((C, M)))
  db_1 = np.array(np.zeros((M, )))
  db_2 = np.array(np.zeros((C, )))
  ...
  for e in range(epoch_size):
    ...
    for j in range(iters_number):
      ...
      dE_da2 = dE_dX * (1-X_2) * X_2
      
      dE_dX = np.dot(W_1.T, dE_da2)
      dE_dW_1 = np.dot(dE_da2, X_1.T)
      dE_db_1 = np.sum(dE_da2, axis=1)

      dW_1 = alpha * dW_1 - learning_rate * dE_dW_1
      dW_2 = alpha * dW_2 - learning_rate * dE_dW_2
      db_1 = alpha * db_1 - learning_rate * dE_db_1
      db_2 = alpha * db_2 - learning_rate * dE_db_2
